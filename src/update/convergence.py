...
def convergence(self):
	...
	self.E = self.E.append(Ei/self.ensembleSize)

	Ri = sum(pow(linalg.vector_norm(r,dim=0),2))\
		/ (linalg.vector_norm(u_exact)**2)
	self.R = self.R.append(Ri/self.ensembleSize)

	ae = mm(mm(sqrt(linalg.pinv(gamma)),G),e)
	AEi = sum(pow(linalg.vector_norm(ae,dim=0),2))\
		/ (linalg.vector_norm(mm(G,m1))**2)
	self.AE = self.AE.append(AEi/self.ensembleSize)
